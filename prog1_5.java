...
public class prog1_5 {
    ...
    static HPnm Bipolarization(HPnm inImg){
        try{
    ...
    static double[][] calculate_moment(boolean a[], double b[], HPnm c) {
        ...
        while (a[figureNumber] == true) {
            ...
            //System.out.println(figureNumber + "番目の重心は(" + figureMoment[figureNumber][0] + ","
            //        + figureMoment[figureNumber][1] + ")");
            figureNumber++;
        }
        return figureMoment;
    }
        ...
        while (a[figureNumber] == true) {
            for (int y = 0; y < b.ysize(); y++)
                for (int x = 0; x < b.xsize(); x++) {
                    ...
                    int value = b.getUnsignedValue(x, y);
                    if (x > 0)
                        value_xb = b.getUnsignedValue(x - 1, y);
                    if (y > 0)
                        value_yb = b.getUnsignedValue(x, y - 1);
                    if (x < b.xsize() - 1)
                        value_xa = b.getUnsignedValue(x + 1, y);
                    if (y < b.ysize() - 1)
                        value_ya = b.getUnsignedValue(x, y + 1);
    ...
    static double[] calculate_circularity(boolean a[], double area[], double length[]) {
        ...
        while (a[figureNumber] == true) {
            figureCricularity[figureNumber] = 4.0 * 3.14159 * area[figureNumber]
                    / (length[figureNumber] * length[figureNumber]);
    ...
    public static void main(String[] args) {
        try {
            ...
            inImg = Bipolarization(inImg);
            inImg.writeVoxels(args[2]);
            int xsize = inImg.xsize();
            int ysize = inImg.ysize();
            ...
            outImg.writeVoxels(args[3]);
            for(int y = 0;y < ysize; y++) for(int x = 0; x < xsize; x++){
                int value = outImg.getUnsignedValue(x,y);
                if(figureCricularity[value] >= 1){
